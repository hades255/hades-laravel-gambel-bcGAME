...
function users_load(json) {
    for(let i = 0; i < json.length; i++) {
        ...
        let e = $('<tr class="kt-datatable__row" style="left: 0px;" data-username="'+user.username+'">' +
           ...
           (user.chat_role === 2 ? 'moderator' : (user.chat_role === 3 ? 'Administrator' : (user.chat_role === 4 ? 'fake' : (user.chat_role === 1 ? 'YouTube' : 'User')))) +
            '</span>' +
            '</div>' +
            '</div>' +
            '</span>' +
            '</td>' +
            '<td class="kt-datatable__cell"><span style="width: 181px; font-family: \'Open Sans\', sans-serif;">'+(parseFloat(user.money).toFixed(2))+' USD.</span></td>' +
...
$(document).ready(function() {
    ...
    '<div class="kt-subheader__group" id="kt_subheader_search">' +
            '<div class="kt-input-icon kt-input-icon--right kt-subheader__search">' +
                '<input oninput="delayedv(\'#generalSearch\', function(v) { send(\'#kt_subheader_search\', \'/admin/search/user/\'+$(\'#generalSearch\').val(), function(response) { search_load(JSON.parse(response)) }) })" type="text" class="form-control" placeholder="To find..." id="generalSearch">' +
