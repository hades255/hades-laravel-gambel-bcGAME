...
function users_load(json) {
    for(let i = 0; i < json.length; i++) {
        ...
        let e = $('<tr class="kt-datatable__row" style="left: 0px;" data-username="'+user.username+'">' +
           ...
           (user.chat_role === 2 ? 'moderator' : (user.chat_role === 3 ? 'Administrator' : (user.chat_role === 4 ? 'Фейк' : (user.chat_role === 1 ? 'YouTube' : 'User')))) +
            '</span>' +
            '</div>' +
            '</div>' +
            '</span>' +
...
$(document).ready(function() {
    ...
    '<div class="kt-subheader__group" id="kt_subheader_search">' +
            '<div class="kt-input-icon kt-input-icon--right kt-subheader__search">' +
                '<input oninput="delayedv(\'#generalSearch\', function(v) { send(\'#kt_subheader_search\', \'/admin/search/fake/\'+$(\'#generalSearch\').val(), function(response) { search_load(JSON.parse(response)) }) })" type="text" class="form-control" placeholder="To find..." id="generalSearch">' +
    ...
    addToolbar(
    '<div class="kt-subheader__group" id="kt_subheader_search">' +
		            '<button type="button" class="btn btn-brand" data-toggle="modal" data-target="#f_settings">Settings</button>' +
	            '<button type="button" class="btn btn-danger" data-toggle="modal" data-target="#new_group">Create multiple fakes</button>' +
            '<button type="button" class="btn btn-success" data-toggle="modal" data-target="#new">Create fake</button>' +
            '<div class="kt-subheader__search" style="width: 85px">' +
                '<input oninput="page($(this).val())" type="text" class="form-control" placeholder="Page">' +
            '</div>' +
