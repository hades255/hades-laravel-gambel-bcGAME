...
function admin_users_load_page(page, callback) {
    send('#users_container', '/admin/payments/'+page, function(response) {
...
function users_load(json) {
    for(let i = 0; i < json.length; i++) {
        ...
        let e = $('<tr class="kt-datatable__row" style="left: 0px;" data-username="'+user.user_id+'">' +
            ...
            '<a class="kt-user-card-v2__name" href="javascript:void(0)" onclick="load(\'/admin/payment?id='+user.id+'\')">'+user.amount+'</a>' +
            '<span class="kt-user-card-v2__desc">' +
           (user.status === 2 ? 'Ошибка' : (user.status === 3 ? 'Ошибка' : (user.status === 4 ? 'Ошибка' : (user.status === 1 ? 'Оплачен': 'Ожидание')))) +
            '</span>' +
            '</div>' +
            '</div>' +
            '</span>' +
            '</td>' +
            '<td class="kt-datatable__cell"><span style="width: 181px; font-family: \'Open Sans\', sans-serif;">'+user.user+'</span></td>' +
            '<td data-autohide-disabled="false" class="kt-datatable__cell">' +
            '<span style="overflow: visible; position: relative; width: 80px;">' +
            '<div class="dropdown">' +
            '<a href="javascript:void(0)" onclick="load(\'/admin/payment?id='+user.id+'\')" class="btn btn-sm btn-clean btn-icon btn-icon-md">' +
...
$(document).ready(function() {
    ...
    '<div class="kt-subheader__group" id="kt_subheader_search">' +
            '<div class="kt-input-icon kt-input-icon--right kt-subheader__search">' +
                '<input oninput="delayedv(\'#generalSearch\', function(v) { send(\'#kt_subheader_search\', \'/admin/search/payment/\'+$(\'#generalSearch\').val(), function(response) { search_load(JSON.parse(response)) }) })" type="text" class="form-control" placeholder="Найти..." id="generalSearch">' +
